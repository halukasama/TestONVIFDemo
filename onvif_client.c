...
#define HEAD "soap.udp://"
#define PORT ":3702"
#define ADDRESS "239.255.255.250"
#define	MULTICAST_ADDRESS(head,body,tail) head body tail
//#define	MULTICAST_ADDRESS       "soap.udp://239.255.255.250:3702"
#define UNICAST_ADDRESS(head,ipaddr, tail) head ipaddr tail
//#define UNICAST_ADDRESS	        "soap.udp://192.168.0.236:3702"
//ONVIF ws-security Anuthon
#define ONVIF_USER              "admin"
#define ONVIF_PASSWORD          "bq111111"

//Change DeviceIPAdress To
#define ONVIF_DIST_NET_WORK_NEW "192.168.0.26"
...
/*! 打开单播流程: Opt:Option  Req:Require  R1o:first[Req]-then[Opt]
...
struct soap* NewSoap(struct SOAP_ENV__Header *header,struct soap* soap, wsdd__ProbeType *req_, wsdd__ScopesType *sScope_)
{
    ...
    printf("[%d][%s][%d][%d][Soap New Error]\n", __LINE__, __func__, soap->recv_timeout, soap->connect_timeout);
    ...
    printf("[%s]\n[%s]\n[%s]\n", soap->header->wsa__Action, soap->header->wsa__MessageID, soap->header->wsa__To);
...
int UserDiscoverDevices(struct soap *soap, wsdd__ProbeType *req, struct __wsdd__ProbeMatches *resp)
{
    ...
    //printf("Probe Address %s", MULTICAST_ADDRESS(HEAD, ADDRESS, PORT));
    int result = soap_send___wsdd__Probe(soap,"soap.udp://192.168.0.236:3702" , NULL, req);
                         ...
                         struct _tds__GetCapabilities *capa_req, struct _tds__GetCapabilitiesResponse *capa_resp)
{
    ...
    *(capa_req->Category) = tt__CapabilityCategory__All;//(enum tt__CapabilityCategory)(tt__CapabilityCategory__Media);
